...
router.get('/checkout_return', (req, res, next) => {
    ...
    paypal.payment.execute(paymentId, details, (error, payment) => {
        ...
        let paymentDetails = '';
        if(error){
            console.log('paypal error', JSON.stringify(error));
...
router.post('/checkout_action', (req, res, next) => {
    ...
    const payment = {
        ...
        transactions: [{
            amount: {
                total: numeral(req.session.totalCartAmount).format('0.00'),
                currency: paypalConfig.paypalCurrency
            },
            description: paypalConfig.paypalCartDescription,
            item_list: {
                "items": Object.values(req.session.cart).map((product) => {
                    return {
                      name: product.title,
                ...
                shipping_address: {
                    recipient_name: `${req.session.customerFirstname} ${req.session.customerLastname}`,
